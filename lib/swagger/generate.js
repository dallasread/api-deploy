...
module.exports = {
    ...
    generateSwagger: function generateSwagger(options, done) {
        ...
        var _ = this,
            template = _.handlebars.compile(_.swagger.templates.resource),
            ...
            for (method in _.routes[path]) {
                try {
                    obj = JSON.parse(template({
                        operationId: Utils.prepPath(_.routes[path][method] + '-' + method.toUpperCase(), {
                            prefix: _.sdk.name,
                            humanize: true
                        }),
                        sdkMethod: Utils.prepPath(path, {
                            camelCase: true
                        }),
                        path: path,
                        method: method,
