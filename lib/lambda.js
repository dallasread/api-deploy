...
module.exports = {
    deployLambda: function deployLambda(route, done) {
        if (!route['x-apiDeploy']) return done();

        var _ = this,
            path = route['x-apiDeploy'].lambda.handler;

        _.logger.log('Compiling Lambda      - ' + route.operationId);

        route.operationId = route.operationId || Utils.prepPath(route['x-apiDeploy'].lambda.handler, {
        ...
        .on('end', function zipComplete() {
            ...
            function deployAPIEndpoint() {
                if (_.shouldDeployAPIGateway()) {
                    _.deployAPIEndpoint(route, done);
                } else {
                    done();
                }
            }

            if (route['x-apiDeploy'].lambda.arn) {
                _.updateLambda(route, zip, deployAPIEndpoint);
            } else {
                _.createLambda(route, zip, deployAPIEndpoint);
            }
        });
    },
    ...
    createLambda: function createLambda(route, zip, done) {
        var _ = this,
            lambda = route['x-apiDeploy'].lambda,
            defaultLambda = _.cfg['x-apiDeploy'].lambda.role,
            options = {
            ...
            function (next) {
                _.AWSLambda.createFunction(options, function(err, data) {
                    if (err) {
                        ...
                        if (data.Role !== defaultLambda.role) route.role = data.Role;
